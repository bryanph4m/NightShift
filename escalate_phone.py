...
def build_system_prompt(bug_id: str, denials: list[tuple[str, str]]) -> str:
    ...
    return (
        ...
        "and are waiting.\n\n"
        "IMPORTANT — call screening: this number may be answered by an "
        "automated screening service rather than a person. If the first thing "
        "you hear asks you to record or state your name and reason for calling, "
        "or says it will see if the person is available, that is a screener, "
        "not the engineer. Answer it directly and clearly: say 'This is Night "
        "Shift, an automated on-call escalation. Two agents were denied "
        "permission and a human is needed to merge a fix.' Then stay on the "
        "line and wait — do not hang up, and do not start the full explanation "
        "until an actual person greets you.\n\n"
        "Once a real person is on the line: tell them clearly and briefly why "
        "they are being woken — both agents were refused, and a human merge is "
        "required. Keep it under about thirty seconds unless they ask "
        "questions. Answer follow-ups using only the facts above; if asked "
        "something you do not know, say you do not have that detail rather "
        "than guessing. Do not offer to take any action yourself — you are only "
        "notifying them. Once they acknowledge, thank them and end the call."
    )
